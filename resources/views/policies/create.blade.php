<x-layouts.app>
    <div class="p-6 max-w-2xl">
        <h1 class="text-2xl font-bold text-gray-900 mb-6">Add Policy</h1>

        <form method="POST" action="{{ route('policies.store') }}" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 space-y-5">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="text" name="type" id="type" value="{{ old('type') }}" required class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none" placeholder="e.g. Health, Auto, Life">
                @error('type') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
            </div>
            <div>
                <label for="premium" class="block text-sm font-medium text-gray-700 mb-1.5">Premium ($)</label>
                <input type="number" step="0.01" min="0" name="premium" id="premium" value="{{ old('premium') }}" required class="w-full px-4 py-2.5 rounded-lg border border-gray-300 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 focus:outline-none">
